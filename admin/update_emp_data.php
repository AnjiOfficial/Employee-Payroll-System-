...
class update_emp_data {
    ...
    public function update() {
        ...
        $this->basic = $_POST['basic'];
		 $this->grade = $_POST['grade'];
		 $this->bname = $_POST['bname'];
		 $this->pan = $_POST['pan'];
        $this->hra = $_POST['hra'];
        $this->acc = $_POST['acc'];
		 $this->gross2 = $_POST['basic']+$_POST['grade'];
		
		 $this->mfactor = $_POST['mfactor'];
		$this->dac= $_POST['mfactor']*($_POST['basic']+$_POST['grade']);
		$this->gross=($_POST['basic']+$_POST['grade'])+($_POST['mfactor']*($_POST['basic']+$_POST['grade']))+$_POST['hra'];
        ...
        if (!$this->profile_pic_parms[name]) {
            $query = mysql_query("Update workers, worker_fixeds, worker_acc set workers.worker_fname='$this->fname', workers.worker_lname='$this->lname', worker_fixeds.basic='$this->basic', worker_fixeds.hra='$this->hra',worker_fixeds.dac='$this->dac',worker_fixeds.gross='$this->gross2',worker_fixeds.grade='$this->grade',worker_fixeds.bname='$this->bname',worker_fixeds.pan='$this->pan' ,worker_acc.accno='$this->acc' where workers.id=worker_fixeds.worker_id and workers.id=worker_acc.id and workers.id='$this->id'");
			 
	  
	   $up_vary= mysql_query("Update worker_varys set rate='$this->newrate' where worker_varys.id='$this->id' and month='$month' and year='$year'");
                    ...
                    if ($this->profile_pic_parms[size] < (1024 * 1024)) {
                        ...
                        if ($width <= 114 && $height <= 144) {
                            if (move_uploaded_file($tmp, $this->profile_pic_parms[path] . $actual_image_name)) {
                                $query = mysql_query("Update workers, worker_fixeds, worker_acc set workers.worker_fname='$this->fname', workers.worker_lname='$this->lname', workers.profilepic='$actual_image_name', worker_fixeds.basic='$this->basic', worker_fixeds.hra='$this->hra',worker_fixeds.gross='$this->gross2', worker_fixeds.dac='$this->dac',worker_acc.accno='$this->acc' where workers.id=worker_fixeds.worker_id and workers.id=worker_acc.id and workers.id='$this->id'");
    ...
    public function delete($id) {
        ...
        $query = mysql_query("Delete w, wf, acc from workers as w, worker_fixeds as wf, worker_acc as acc where w.id=wf.worker_id and w.id=acc.id and w.id='$this->id'");
		   $query2= mysql_query("INSERT INTO workerold_varys
                     SELECT * FROM worker_varys as a 
                  WHERE a.id='$this->id'");
		$query2 = mysql_query("Delete a from worker_varys as a where a.id='$this->id'");
		$query3 = mysql_query("Delete b from nett as b where b.id='$this->id'");
