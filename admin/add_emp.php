...
class add_emp {
    ...
    public function addemp() {
		  $this->dac= $_POST['dac']*($_POST['basic']+$_POST['grade']);
		$this->bname=$_POST['bname'];
		$this->pan=$_POST['pan'];
        $this->emp_id= $_POST['emp_id'];
        $this->fname= $_POST['fname'];
        $this->lname= $_POST['lname'];
        $this->gross= $_POST['basic'];
        $this->hra= $_POST['hra'];
        $this->acc= $_POST['acc'];
		$this->grade= $_POST['grade'];
		$this->category= $_POST['cat'];
		$this->mfactor=$_POST['dac'];
		
		$this->total=$_POST['basic']+$_POST['grade'];
        ...
        if(!$this->profile_pic_parms[name]) {
			...
			elseif (trim($this->grade=='')) {
                ...
                if(!$this->query[0]) {
                    ...
                    $this->query[1]= mysql_query("Insert into worker_fixeds (worker_id, basic, hra,grade,gross,category,dac,bname,pan,mfactor) values ('$this->emp_id', '$this->gross', '$this->hra',' $this->grade',' $this->total','$this->category','$this->dac','$this->bname','$this->pan','$this->mfactor')");
                    if(!$this->query[1]) {
                        header("location:../checklogin/qerror.php?error=".base64_encode(mysql_error()));
                    }
                    else {
